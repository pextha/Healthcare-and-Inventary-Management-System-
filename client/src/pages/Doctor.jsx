...
import { CalendarDays, ClipboardList, AlertCircle, Stethoscope } from "lucide-react";
import DashboardLayout from "../components/layout/DashboardLayout";
import { DOCTOR_NAV } from "../config/navConfig";
import { useAuth } from "../context/useAuth";
import DoctorStatCard from "../components/ui/DoctorStatCard";
import DoctorUpcomingAppointmentCard from "../components/ui/DoctorUpcomingAppointmentCard";
import DoctorMessagesPreviewCard from "../components/ui/DoctorMessagesPreviewCard";
...
function Doctor() {
  ...
  const displayName = userFullName ? userFullName.split(" ")[0] : "Doctor";

  // null = loading, "INACTIVE" = not approved, "ERROR" = fetch failed, object = data
  const [status, setStatus] = useState(null);
  const [stats, setStats] = useState({ upcoming: 0, pending: 0, patients: 0 });
  ...
  const loadDashboard = useCallback(async () => {
    ...
    try {
      ...
      const patients = (pregnancyRes.data?.data ?? []).length;

      setStats({ upcoming, pending, patients });
      setStatus("OK");
    } catch (err) {
      if (
      ...
      <div className="w-full mx-auto max-w-3xl">
        ...
        <header className="mb-8 text-center">
          ...
          <p className="mt-2 text-sm md:text-base text-text-secondary dark:text-text-muted">
            Here's a quick look at your current workload.
          </p>
        ...
        {(isLoading || status === "OK") && (
          <>
            {/* Stat summary row */}
            <section aria-label="Doctor overview stats">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <DoctorStatCard
                  icon={<Stethoscope size={13} />}
                  label="Active Patients"
                  value={isLoading ? undefined : stats.patients}
                  loading={isLoading}
                  onClick={isLoading ? undefined : () => navigate("/doctor/patients")}
                />
                <DoctorStatCard
                  ...
                  loading={isLoading}
                  onClick={isLoading ? undefined : () => navigate("/doctor/appointments")}
                />
                <DoctorStatCard
