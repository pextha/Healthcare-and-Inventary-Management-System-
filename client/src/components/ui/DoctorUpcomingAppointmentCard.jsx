...
export default function DoctorUpcomingAppointmentCard() {
  const [appts, setAppts]               = useState([]);  // up to 2 priority appointments
  const [pendingCount, setPendingCount] = useState(0);   // PENDING + RESCHEDULE_REQUESTED
  const [totalActive, setTotalActive]   = useState(0);   // all non-completed, non-cancelled
  ...
  const fetchAppointments = useCallback(async () => {
    ...
    try {
      ...
      // Pending requests needing doctor's attention
